...
// Constructor with separate on/off durations:
LEDExpiringToggler2::LEDExpiringToggler2(uint8_t pin, int64_t lifetimeMs, unsigned int toggleDurationOnMs, unsigned int toggleDurationOffMs, bool highIsOn)
    : pin(pin),
      highIsOn(highIsOn),
      toggler(lifetimeMs, toggleDurationOnMs, toggleDurationOffMs) {
  pinMode(pin, OUTPUT);
  setLedOff_();
}

// Constructor with single interval (backward compatibility):
LEDExpiringToggler2::LEDExpiringToggler2(uint8_t pin, int64_t lifetimeMs, unsigned int toggleIntervalMs, bool highIsOn)
    : pin(pin),
      highIsOn(highIsOn),
      toggler(lifetimeMs, toggleIntervalMs, toggleIntervalMs) {
  pinMode(pin, OUTPUT);
  setLedOff_();
}

// Efficient: pass current time in microseconds (recommended)
void LEDExpiringToggler2::checkToggleLED(int64_t currentMicros) {
  if (!toggler.checkToggle(currentMicros)) return;
  toggleLED_();
}

// Convenience: calls esp_timer_get_time() internally (less efficient)
void LEDExpiringToggler2::checkToggleLED() {
  // we let the internal toggler call `esp_timer_get_time()` instead of _always_ calling it here,
  // because `toggler.checkToggle()` shortcuts the expensive `esp_timer_get_time()` call in various cases
  if (!toggler.checkToggle()) return;
  toggleLED_();
...
void LEDExpiringToggler2::toggleLED_() {
  // state has changed, so query new state and set LED accordingly
  if (toggler.isCurrentStateOn()) {
    setLedOn_();
  } else {
...
void LEDExpiringToggler2::activate(unsigned int delayMs /* = 0 */) {
  // Calling activate() itself leaves the LED off, but activates the LED toggling cycle (after specified delay).
  // The next call to `checkToggleLED()` (after `delayMs` milliseconds), will turn the LED on.
  // This is exactly how the underlying FrequencyToggler2 works.
  toggler.activate(delayMs);
}

void LEDExpiringToggler2::expire() {
  toggler.expire();
  setLedOff_();
}

bool LEDExpiringToggler2::isExpired() { return toggler.isExpired(); }

void LEDExpiringToggler2::setLedOn_() {
  if (highIsOn) {
    digitalWrite(pin, HIGH);
  } else {
    digitalWrite(pin, LOW);
...
void LEDExpiringToggler2::setLedOff_() {
  if (highIsOn) {
    digitalWrite(pin, LOW);
  } else {
    digitalWrite(pin, HIGH);
